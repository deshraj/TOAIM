...
function loadBaseData() {
    ...
    gt_answer_list = cur_scene_dict["gt_answer_list"];
    gt_acc_list = cur_scene_dict["gt_acc_list"];

    gt_list = [];
    for (var i = 0; i < q_id_list.length; ++i) {
        gt_list[i] = (gt_acc_list[i] >= 1) ? "Correctly" : "Wrongly";
    }
...
function validate_scene(alert=true) {
    ...
    for (var val in scene_choice_dict) {
        // if (scene_choice_dict[val]["choice"] != undefined) {
        //     // console.log(scene_choice_dict[val]["choice"]);
        // }
    }
    if (choice === 'correctly' || choice === 'wrongly') {
        // valid choice
    }
    // invalid choice 
    else {
